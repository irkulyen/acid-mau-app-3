...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    console.log("[Auth] authenticateRequest - authHeader:", authHeader ? "present" : "missing");
    ...
    if (token) {
      ...
      try {
        ...
        if (jwtPayload) {
          ...
          console.log("[Auth] Loading user by ID:", jwtPayload.userId);
          const user = await db.getUserById(jwtPayload.userId);
          if (user) {
            console.log("[Auth] ✓ Authenticated via JWT token, userId:", user.id);
            return user;
          } else {
            console.log("[Auth] ✗ User not found in DB for userId:", jwtPayload.userId);
          }
        } else {
          console.log("[Auth] ✗ JWT payload is null");
        }
      } catch (error) {
        console.log("[Auth] ✗ JWT verification failed:", error);
      }
    } else {
      console.log("[Auth] No Bearer token found, trying OAuth session cookie");
    }

    // Fall back to OAuth session cookie authentication
    const cookies = this.parseCookies(req.headers.cookie);
    const cookieToken = cookies.get(COOKIE_NAME);
    
    // If cookie contains a JWT token (email/password login), verify it
    if (cookieToken && !token) {
      console.log("[Auth] Trying JWT verification from cookie...");
      try {
        const { verifyToken } = await import("../auth-helpers");
        const jwtPayload = await verifyToken(cookieToken);
        
        if (jwtPayload) {
          console.log("[Auth] Loading user by ID from cookie JWT:", jwtPayload.userId);
          const user = await db.getUserById(jwtPayload.userId);
          if (user) {
            console.log("[Auth] ✓ Authenticated via cookie JWT, userId:", user.id);
            return user;
          }
        }
      } catch (error) {
        console.log("[Auth] Cookie JWT verification failed, trying OAuth session:", error);
      }
    }
    
    // Try OAuth session cookie
    const sessionCookie = token || cookieToken;
